...
const language = require("../src/languages/mcfunction");
const fs = require("fs");
const path = require("path");
const languageName = "mcfunction";
...
describe("highlight " + languageName, () => {
  ...
  it("highlights language file", () => {
    // read the test data
    const input = fs.readFileSync(path.resolve(__dirname, testFileSourcePath), "utf-8");
    ...
    // const expected = fs.readFileSync(path.resolve(__dirname, testFileExpectedPath), "utf-8");
    // expect(result.value).toBe(expected);
  });
});
